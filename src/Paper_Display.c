...
void INIT_SSD1673()
{
	...
	DIS_IMG(PIC_BLACK);         // 全黑到全白清屏，这样可防止开机出现花屏的问题
...
void write_lut_partial() {
	...
	SPI4W_WRITECOM(0x32);//write LUT register
	for(i=0;i<29;i++)
        SPI4W_WRITEDATA(partial_lut[i]);//write LUT register
}
...
void DIS_() {
  ...
  DELAY_S(5);
  pcnt = 0;                 // 复位或保存提示字节序号
  for(col=0; col<10; col++)   // 总共250 GATE列  // send 128x250bits ram 2D13
  {
